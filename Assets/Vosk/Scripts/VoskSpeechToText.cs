...
public class VoskSpeechToText : MonoBehaviour
{
	...
	//Guards the recognizer while it is being swapped at runtime.
	private readonly object _recognizerLock = new object();

	/// <summary>Vosk 是否已完成初始化（模型解压、识别器就绪）</summary>
	...
	/// <summary>
	/// 运行时切换识别语法：限定词汇表可大幅提升这些词的识别准确度。
	/// 传 null 或空列表 = 恢复自由识别（模型词表内任意词）。
	/// 在后台线程与音频处理互斥，线程安全。
	/// </summary>
	public void SetKeyPhrases(List<string> keyPhrases)
	{
		...
		// 旧识别器已无引用，安全释放
		old?.Dispose();
		Debug.Log("Recognizer grammar updated: " + (string.IsNullOrEmpty(_grammar) ? "<free-form>" : _grammar));
	}
	...
	private async Task ThreadedWork()
	{
		...
		while (_running)
		{
			if (_threadedBufferQueue.TryDequeue(out short[] voiceResult))
			{
				...
				lock (_recognizerLock)
				{
					final = _recognizer.AcceptWaveform(voiceResult, voiceResult.Length);
					if (final)
